...
class TaskViewController: UITableViewController {
  ...
  func configureText(for cell: UITableViewCell, with item: TaskItem) {
    if let checkmarkCell = cell as? TaskTableViewCell {
      checkmarkCell.todoTextLabel.text = item.text
    }
  }
  
  func configureCheckmark(for cell: UITableViewCell, with item: TaskItem) {
    guard let checkmarkCell = cell as? TaskTableViewCell else {
      return
    }
    if item.checked {
...
extension TaskViewController: ItemDetailViewControllerDelegate {
  func itemDetailViewControllerDidCancel(_ controller: ItemDetailViewController) {
    navigationController?.popViewController(animated: true)
  }

  func itemDetailViewController(_ controller: ItemDetailViewController, didFinishAdding item: TaskItem) {
    navigationController?.popViewController(animated: true)
  ...
  func itemDetailViewController(_ controller: ItemDetailViewController, didFinishEditing item: TaskItem) {
